...
class CLineStrip : public CShape
{
public:
	DECLARE_SERIAL( CLineStrip )

	CLineStrip(void):
		m_nStyle(PS_SOLID),m_nWidth(1), points(0), num(0) { };
	CLineStrip(CPoint *ptList, int ptCount):
	...
	bool Modify();
	CShape* Clone() {
		CLineStrip *p = new CLineStrip(points,num);
		p->m_nColor = m_nColor;
		p->m_nStyle = m_nStyle;
		p->m_nWidth = m_nWidth;
		return p;;
...
private:
	CLineStrip(const CLineStrip&);
	CLineStrip& operator=(const CLineStrip&);
	CPoint *points;
	int num;
	CPoint m_p1,m_p2;
