...
#define PKT_TOTAL_LAN_MAX (236)  /*报文最大长度*/
...
uint8_t *pucMsgType, uint32_t *pulUserDataLen, uint8_t *pucUserDataBuf)
{
	...
	else
	{
		...
		{
			ucType = aucBuf[0];  /*获取消息类型*/
            ulUserDataLen = aucBuf[3]; /*获取用户数据长度*/
            ucSeq = S_ntohs((uint16_t)(aucBuf[1])); /*消息序号转换字节序*/
            /*消息类型检查*/
            if (ucType!=PKT_PRI_LOW && ucType!=PKT_PRI_MID && ucType!=PKT_PRI_HIGH)
            {
                lRet = S_ERROR;
            }
            else if (ulUserDataLen > PKT_USERDATA_MAX || ulUserDataLen != (ulDataLen-PKT_HEAD_LEN)) /*消息长度等于或小于DRV_Receive函数获取的长度-4；消息中长度小于等于232*/
            {
                lRet = S_ERROR;
		
            }
            else if (ucSeq中包序号不正确) 
            {
                lRet = S_ERROR;
            }
            else
            {
                ...
                *pucMsgType = ucType;
                /*消息长度赋值出参*/
                *pulUserDataLen = ulUserDataLen;
                /*消息内容赋值出参*/
                S_MemCpy(pucUserDataBuf,aucBuf+PKT_HEAD_LEN, ulUserDataLen);
                lRet = S_OK;
			}
		}
	}
